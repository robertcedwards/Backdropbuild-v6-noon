...
export function classifyRepo(language, files) {
    ...
    for (const level of levels) {
        ...
        for (const file of files) {
          ...
          if (patterns.useAst) {
            const ast = parse(content, patterns.parseOptions);
            traverse(ast, patterns.astVisitors);
          }
    
          for (const [patternName, detector] of Object.entries(patterns[level])) {
            if (detector(content, file) && !usedElements.includes(patternName)) {
              usedElements.push(patternName);
            }
          }
        }
